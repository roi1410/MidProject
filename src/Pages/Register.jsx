...
export const Register = () => {
  ...
  useEffect(() => {
     ...
     if (RegisterUser) {
      if (!UserLocalStorege?.some((element)=>element.Username===RegisterUser.Username)) {
      ...
      console.log("chage the user name ");
     }
    }
  }, [RegisterUser]);
    ...
    <div className="relative flex flex-col rounded-xl bg-cyan-50 bg-transparent bg-clip-border text-gray-700 shadow-none md:hidden">
      ...
      >
        <div className="mb-4 flex flex-col gap-6">
          ...
          <div className="relative h-11 w-full min-w-[200px]">
            <input
              ...
              {...register("Username",{required:"Username is required",validate:{
               maxLength:(v)=> v.length<20||"the Username shold be lower than 20 chars",
               minLength:(v)=>v.length>4|| "the username need to be higher than 4 chars",
               pattern:(v)=>containsLettersAndNumbers(v) ||"the username need to contain letters and numbers",
               exsistingUser:(v)=>!UserLocalStorege?.some((element)=>element.Username===v)||"this Username is alrady taken"
              }
              })}
          ...
          <div className="relative h-11 w-full min-w-[200px]">
            <input
              {...register("password",{required:"passowrd is required",validate:{
                pattern:(v)=>validatePassword(v)||"the password shold contain capital letters numbers and regular letters"
